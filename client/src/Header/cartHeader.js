import React, { Component } from 'react'
import {Row,Col,Image,Navbar} from 'react-bootstrap'
import 'bootstrap/dist/css/bootstrap.min.css';
import {Link} from 'react-router-dom'

export class cartHeader extends Component {

    handlebreadcrumb=()=>{
        if(window.location.pathname==='/cart'){
           return( <ol className="breadcrumb mt-1 bg-white" style={{marginLeft:"300px"}}>
                        <li className=" bg-white"><a href="#"
                                class="text-decoration-underline  text-success">BAG</a></li>
                                 ---------------------------
                        <li className=" bg-white"><a href="#"
                                className="text-decoration-none text-muted ">ADDRESS</a></li>
                                ---------------------------
                        <li className=" bg-white"><a href="#"
                                className="text-decoration-none text-muted ">PAYMENT</a>
                        </li>
            </ol>)
        }
        if(window.location.pathname==='/cart/address'){
            return( <ol className="breadcrumb mt-1 bg-white" style={{marginLeft:"300px"}}>
                         <li className=" bg-white"><a href="#"
                                 class="text-decoration-none text-muted">BAG</a></li>
                                 ---------------------------
                         <li className=" bg-white"><a href="#"
                                 className="text-decoration-underline  text-success ">ADDRESS</a></li>
                                 ---------------------------
                         <li className=" bg-white"><a href="#"
                                 className="text-decoration-none text-muted ">PAYMENT</a>
                         </li>
             </ol>)
         }
         if(window.location.pathname==='/cart/order'||window.location.pathname==='/cart/orderdetail/:id'){
            return( <div></div>)
         }


    }
    render() {
        var renderBreadCrumb=this.handlebreadcrumb()
        return (
            <div>
                    <header>
                        <Row>
                    <nav class="navbar navbar-expand-sm bg-white">
                        <Col lg={1}>
                         <a class="navbar-brand " href="/" style={{marginLeft: "100px"}}>
                <img src="data:image/png;base64,"
                    alt="Myntra Logo" title="Logo" height="55" width="55" class="ms-1 d-inline-block rounded-pill" />
            </a>
            </Col>
            <Col lg={7}>
            <div className="mx-5 bg-white ">
                <nav aria-label="breadcrumb">
                   {renderBreadCrumb}
                </nav>
            </div>
            </Col>
            <Col lg={4}>
            <div className="" style={{marginLeft:"300px"}}>
                <span className="ms-1 text-dark fw-bold"><img src="https://constant.myntassets.com/checkout/assets/img/sprite-secure.png" width="40px" height="40px"
                       /> 100% SECURE</span>
            </div>
            </Col>

        </nav>
        </Row>
    </header>
            </div>
        )
    }
}

export default cartHeader
